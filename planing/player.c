	...
	if (has(OL_NR(GetNthNode(PersonsList, current)), Ability_Aufpassen))
   {
      ...
      if (Search.EscapeBits & FAHN_QUIET_ALARM)
         watch = tcWatchDogWarning(OL_NR(GetNthNode(PersonsList, current)));
			...
			watch = tcWrongWatchDogWarning(OL_NR(GetNthNode(PersonsList, current)));
...
static void plPlayerAction(void)
{
	...
	if (!(PD.timer % patroCounter))
	{
		...
		if ((CalcRandomNr(0, 30 * (270 - tcRGetGRate(PD.bldObj) / 2 + 1))) == 0)
		{
			...
			if (tcAlarmByPatrol(PD.bldObj, PD.changeCount, PD.totalCount ,PD.patrolCount))
	...
	for (i = 0; i < PersonsNr; i++)
	{
		if ((i >= BurglarsNr) || !PD.handlerEnded[i])
		{
			...
			else if (PD.action = NextAction(plSys))
			{
				...
				{
					ubyte j, dir = livGetViewDirection(Planing_Name[i]);
					...
					for (j = 0; j < BurglarsNr; j++)
					{
						if (tcGuardDetectsGuy(Planing_GuardRoomList[i - BurglarsNr], xpos, ypos, dir, Planing_Name[i], Planing_Name[j]))
						{
							...
							PD.guardKO[((Police)dbGetObject(OL_NR(GetNthNode(PersonsList, i))))->LivingID - BurglarsNr] = 3;
							break;
						}
					}
					#endif
				...
				switch (PD.action->Type)
				{
					case ACTION_GO:
						if (i < BurglarsNr)
						{
							...
							{
								plMove(i, ActionData(PD.action, struct ActionGo *)->Direction);
								...
								if (livCanWalk(Planing_Name[i]))
								{
									...
									Search.Exhaust[i]  = tcGuyInAction(OL_NR(GetNthNode(PersonsList, i)), Search.Exhaust[i]);
									Search.WalkTime[i]++;

									if (i == CurrentPerson)
									{
										lsInitScrollLandScape(ActionData(PD.action, struct ActionGo *)->Direction, LS_SCROLL_PREPARE);
										DoScrolling = 1;
									}
								}
								else
					...
					case ACTION_SIGNAL:
						if (i < BurglarsNr)
						{
							...
							if (ActionStarted(plSys))
							{
								...
								InitSignal(plSys, OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionSignal *)->ReceiverId);
							...
							{
								struct plSignal *sig = IsSignal(plSys, OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionSignal *)->ReceiverId);
					...
					case ACTION_WAIT_SIGNAL:
						if (i < BurglarsNr)
						{
							struct plSignal *sig = IsSignal(plSys, ActionData(PD.action, struct ActionWaitSignal *)->SenderId, OL_NR(GetNthNode(PersonsList, i)));
					...
					case ACTION_USE:
						if (i < BurglarsNr)
						{
							...
							if (ActionStarted(plSys))
							{
								if (plIsStair(ActionData(PD.action, struct ActionUse *)->ItemId))
                        ...
                        else
                        {
                           ...
                           {
                              if (plObjectInReach(i, ActionData(PD.action, struct ActionUse *)->ItemId))
										{
                                 PD.currLoudness[i] = tcGetToolLoudness(OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionUse *)->ToolId, Item_Wache);
                                 PD.guardKO[((Police)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->LivingID - BurglarsNr] = 1;
                                 PD.changeCount += 5;

                                 realTime            = tcGuyUsesToolInPlayer(OL_NR(GetNthNode(PersonsList, i)), PD.bldObj, ActionData(PD.action, struct ActionUse *)->ToolId, Item_Wache, needTime / PLANING_CORRECT_TIME);
                           ...
                           {
                              if (!CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId), Const_tcLOCK_UNLOCK_BIT))
											PD.currLoudness[i] = tcGetToolLoudness(OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionUse *)->ToolId, ((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type);
                              else if ((((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type == Item_Fenster))
                                 PD.currLoudness[i] = PLANING_LOUDNESS_STD;
                              else if (!plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId))
                              {
                              ...
                              #ifndef PLAN_IS_PERFECT
                              if (tcGetDanger(OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionUse *)->ToolId, ((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type))
                                 PD.handlerEnded[i] = 2;
                              #endif

                              realTime = tcGuyUsesToolInPlayer(OL_NR(GetNthNode(PersonsList, i)), PD.bldObj, ActionData(PD.action, struct ActionUse *)->ToolId, ((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type, needTime / PLANING_CORRECT_TIME);
                           }
									...
									if (realTime)
                           {
                              ...
                              IncCurrentTimer(plSys, realTime * PLANING_CORRECT_TIME - needTime, 0);
                           }

                           plPersonLearns(OL_NR(GetNthNode(PersonsList, i)), ActionData(PD.action, struct ActionUse *)->ToolId);
								}
                     }

                     #ifndef PLAN_IS_PERFECT
							if (!(Search.EscapeBits & FAHN_ALARM_ALARM) && tcAlarmByTouch(ActionData(PD.action, struct ActionUse *)->ItemId))
                     {
                        Search.DeriTime   += PLANING_DERI_ALARM;
                     ...
                     if (ActionEnded(plSys))
                     {
								if (!plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId))
                        {
                           if (plIsStair(ActionData(PD.action, struct ActionUse *)->ItemId))
                           {
                              if (i == CurrentPerson)
                           ...
                           else if (dbIsObject(ActionData(PD.action, struct ActionUse *)->ItemId, Object_Police))
									{
                              if (tcKillTheGuard(OL_NR(GetNthNode(PersonsList, i)), PD.bldId))
                                 PD.guardKO[((Police)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->LivingID - BurglarsNr] = 2;
                              else
                              {
                                 if (!(Search.EscapeBits & FAHN_ALARM_GUARD))
                                 {
                                    plSay("PLAYER_GUARD_ALARM_5", ((Police)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->LivingID);
												inpSetWaitTicks(INP_AS_FAST_AS_POSSIBLE);
                                    Search.DeriTime   += PLANING_DERI_ALARM;
                                    Search.EscapeBits |= FAHN_ALARM|FAHN_ALARM_GUARD;
                                 }
                                 ...
                                 PD.guardKO[((Police)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->LivingID - BurglarsNr] = 3;
                              }
									}
                           else
                           {
                              if (!CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId), Const_tcLOCK_UNLOCK_BIT))
                              {
                                 PD.changeCount++;
											...
											lsSetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId, Const_tcLOCK_UNLOCK_BIT, 1);

                                 if (((Tool)dbGetObject(ActionData(PD.action, struct ActionUse *)->ToolId))->Effect & Const_tcTOOL_OPENS)
                                 {
                                    lsSetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId, Const_tcOPEN_CLOSE_BIT, 1);
                                    plCorrectOpened((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId), 1);
                                 }

                                 if (ActionData(PD.action, struct ActionUse *)->ToolId == Tool_Stechkarte)
												tcRefreshTimeClock(PD.bldId, ActionData(PD.action, struct ActionUse *)->ItemId);
                              }
                              else
                              {
											if ((((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type == Item_Fenster))
                                 {
												uword xpos, ypos;

                                    lsWalkThroughWindow((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId), livGetXPos(Planing_Name[i]), livGetYPos(Planing_Name[i]), &xpos, &ypos);
                        ...
                        else
                        {
									...
									if (CHECK_STATE(state, Const_tcON_OFF))
                           {
                              ...
                              if (plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId) == PLANING_POWER)
                              {
											lsSetSpotStatus(ActionData(PD.action, struct ActionUse *)->ItemId, LS_SPOT_ON);
                           ...
                           else
                           {
                              ...
                              switch (plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId))
                              {
                                 case PLANING_ALARM_TOP3:
                                    if (PD.alarmTimer)
                                       PD.alarmTimer = min(PD.alarmTimer,PLANING_ALARM_TOP3);
                                    else
                                       PD.alarmTimer = PLANING_ALARM_TOP3;
											break;

                                 case PLANING_POWER:
                                    lsSetSpotStatus(ActionData(PD.action, struct ActionUse *)->ItemId, LS_SPOT_OFF);
												lsShowAllSpots(PD.realTime, LS_ALL_INVISIBLE_SPOTS);

												#ifndef PLAN_IS_PERFECT
                                    if (!(Search.EscapeBits & FAHN_ALARM_POWER) && tcAlarmByPowerLoss(ActionData(PD.action, struct ActionUse *)->ItemId))
                                    {
                        ...
                        plRefresh(ActionData(PD.action, struct ActionUse *)->ItemId);
                        plDisplayInfo();
                     }
                  }
               break;
						...
						if (i < BurglarsNr)
                  {
                     ...
                     if (ActionStarted(plSys))
                        PD.changeCount++;
							...
							if (Ask(dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), hasLoot(i), dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId)) && plObjectInReach(i, ActionData(PD.action, struct ActionTake *)->ItemId))
                     {
                        lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId, Const_tcIN_PROGRESS_BIT, 1);
                        ...
                        if (ActionEnded(plSys))
                           ...
                           lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId, Const_tcIN_PROGRESS_BIT, 0);

                           if ((ActionData(PD.action, struct ActionTake *)->ItemId >= 9701) && (ActionData(PD.action, struct ActionTake *)->ItemId <= 9708))
                           {
                              lsRemLootBag(ActionData(PD.action, struct ActionTake *)->ItemId);
                              Planing_Loot[ActionData(PD.action, struct ActionTake *)->ItemId-9701] = 0;
									}
                           else
                           {
                              if (CHECK_STATE(lsGetObjectState (ActionData(PD.action, struct ActionTake *)->ItemId), Const_tcTAKE_BIT))
                              {
                                 lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId, Const_tcACCESS_BIT, 0);
                                 lsTurnObject((LSObject)dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), LS_OBJECT_INVISIBLE, LS_NO_COLLISION);
                              }
									}

                           UnSet(dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), hasLoot(i), dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId));
                           ...
                           if (Ask(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId)))
                           {
                              ...
                              SetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId), newValue);

                           plRefresh(ActionData(PD.action, struct ActionTake *)->ItemId);
                           plDisplayInfo();
                        }
                     }
                     else
               ...
               case ACTION_DROP:
                  if (i < BurglarsNr)
                  {
                     ...
                     if (Ask(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId)))
                     {
                        lsSetObjectState(ActionData(PD.action, struct ActionDrop *)->ItemId, Const_tcIN_PROGRESS_BIT, 1);
                        ...
                        if (ActionEnded(plSys))
                        {
                           ...
                           if ((ActionData(PD.action, struct ActionDrop *)->ItemId >= 9701) && (ActionData(PD.action, struct ActionDrop *)->ItemId <= 9708))
                           {
                              lsAddLootBag(livGetXPos(Planing_Name[i]), livGetYPos(Planing_Name[i]), ActionData(PD.action, struct ActionDrop *)->ItemId-9700);
                              Planing_Loot[ActionData(PD.action, struct ActionDrop *)->ItemId-9701] = 1;
									}
                           else
                           {
                              if (CHECK_STATE(lsGetObjectState (ActionData(PD.action, struct ActionTake *)->ItemId), Const_tcTAKE_BIT))
                                 lsTurnObject((LSObject)dbGetObject (ActionData(PD.action, struct ActionDrop *)->ItemId), LS_OBJECT_VISIBLE, LS_COLLISION);
                           }

                           SetP(dbGetObject(ActionData(PD.action, struct ActionDrop *)->ItemId), hasLoot(i), dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId), newValue);
                           UnSet(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId));

                           plRefresh(ActionData(PD.action, struct ActionDrop *)->ItemId);
                           plDisplayInfo();
                        }
							}
                     else
               ...
               break;
                  ...
                  if (ActionStarted(plSys))
                  {
                     ...
                     {
                        #ifndef PLAN_IS_PERFECT
                        if (CHECK_STATE (lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId), Const_tcOPEN_CLOSE_BIT))
                        {
                           if (!(Search.EscapeBits & FAHN_ALARM_GUARD))
									{
                  ...
                  if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionOpen *)->ItemId) || CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId), Const_tcLOCK_UNLOCK_BIT))
                  {
                     if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionOpen *)->ItemId) || !CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId), Const_tcOPEN_CLOSE_BIT))
                     {
                        if (i < BurglarsNr)
                        {
									PD.currLoudness[i] = PLANING_LOUDNESS_OPEN_CLOSE;
                           Search.WorkTime[i]++;
                           lsSetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId, Const_tcIN_PROGRESS_BIT, 1);
								...
								if (ActionEnded(plSys))
                        {
                           ...
                           {
                              PD.changeCount++;
                              lsSetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId, Const_tcIN_PROGRESS_BIT, 0);
									}
									...
									plCorrectOpened((LSObject)dbGetObject(ActionData(PD.action, struct ActionOpen *)->ItemId), 1);

                           plRefresh(ActionData(PD.action, struct ActionOpen *)->ItemId);
                           plDisplayInfo();
                        }
                     }
                     else
               ...
               case ACTION_CLOSE:
                  ...
                  if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionClose *)->ItemId) || CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId), Const_tcLOCK_UNLOCK_BIT))
                  {
                     if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionClose *)->ItemId) || CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId), Const_tcOPEN_CLOSE_BIT))
                     {
                        if (i < BurglarsNr)
                        {
                           PD.currLoudness[i] = PLANING_LOUDNESS_OPEN_CLOSE;
                           Search.WorkTime[i]++;
                           lsSetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId, Const_tcIN_PROGRESS_BIT, 1);
                        ...
                        if (ActionEnded(plSys))
                        {
                           if (i < BurglarsNr)
									{
                              PD.changeCount--;
										lsSetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId, Const_tcIN_PROGRESS_BIT, 0);
                           }

                           lsSetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId, Const_tcOPEN_CLOSE_BIT, 0);

                           plCorrectOpened((LSObject)dbGetObject(ActionData(PD.action, struct ActionClose *)->ItemId), 0);

									plRefresh(ActionData(PD.action, struct ActionClose *)->ItemId);
                           plDisplayInfo();
                        }
                     }
                     else
               ...
               case ACTION_CONTROL:
                  #ifndef PLAN_IS_PERFECT
                  if (tcGuardChecksObject((LSObject)dbGetObject(ActionData(PD.action, struct ActionControl *)->ItemId)))
						{
							if (!(Search.EscapeBits & FAHN_ALARM_GUARD))
							{
						...
						if (((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type == Item_Stechuhr)
							tcRefreshTimeClock(PD.bldId, ActionData(PD.action, struct ActionUse *)->ItemId);
					break;
				}
			}
      ...
      if (!(l = LoadSystem(fh, plSys)))
      {
         if (!(l = plLoadTools(fh)))
         {
            PD.ende        = 0;
				...
				PD.patrolCount = 0;
				PD.alarmTimer  = 0L;
				PD.isItDark    = ((LSArea)dbGetObject(lsGetActivAreaID()))->uch_Darkness;
				PD.sndState		= 1;
            ...
            while (!PD.ende)
            {
               ...
               inpTurnESC(0);
					...
					activ = Menu(menu, bitset, activ, NULL, INP_AS_FAST_AS_POSSIBLE);
                  ...
                  case PLANING_PLAYER_PERSON_CHANGE:
                     ...
                     if (choice1 != GET_OUT)
                     {
                        if (livWhereIs(Planing_Name[choice1]) != lsGetActivAreaID())
                        {
                           ...
                           lsInitActivArea(livWhereIs(Planing_Name[choice1]), livGetXPos(Planing_Name[choice1]), livGetYPos(Planing_Name[choice1]), Planing_Name[choice1]);
                        }
                  ...
                  case PLANING_PLAYER_RADIO_ONE:
                     if (BurglarsNr > 2)
                     {
                        ...
                        dbRemObjectNode(BurglarsList, 0L);
								...
								choice1 = CurrentPerson ? OL_NR(GetNthNode(BurglarsList, 0)) : OL_NR(GetNthNode(BurglarsList, 1));
            ...
            if (!(Search.EscapeBits & FAHN_SURROUNDED) && !(Search.EscapeBits & FAHN_ESCAPE) && !plLivingsPosAtCar(objId))
            {
            ...
            for (i = 0; i < BurglarsNr; i++)
               ...
               AskAll(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, BuildObjectList);
            ...
            if ((!(Search.EscapeBits & FAHN_ALARM)) && (!(Search.EscapeBits & FAHN_QUIET_ALARM)) && (!(Search.EscapeBits & FAHN_ESCAPE)))
