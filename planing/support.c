...
ubyte plLivingsPosAtCar(ulong bldId)
{
	...
	for (i = 0; i < BurglarsNr; i++)
   {
      ...
      if (((xpos < (carxpos - PLANING_AREA_CAR)) || (xpos > (carxpos + PLANING_AREA_CAR))) && ((ypos < (carypos - PLANING_AREA_CAR)) || (ypos > (carypos + PLANING_AREA_CAR))))
      {
         atCar = 0;
         break;
      }
...
ubyte plAllInCar(ulong bldId)
{
   ...
   if (maxPerson != realCurrentPerson)
   {
      ...
      plSync(PLANING_ANIMATE_NO, GetMaxTimer(plSys), GetMaxTimer(plSys) - oldTimer, 1);
   }
...
void plMove(ulong current, ubyte direction)
{
   ...
   {
		case DIRECTION_LEFT:
         livAnimate(Planing_Name[current], ANM_MOVE_LEFT, -1*LS_STD_SCROLL_SPEED, 0);
      break;

      case DIRECTION_RIGHT:
         livAnimate(Planing_Name[current], ANM_MOVE_RIGHT, 1*LS_STD_SCROLL_SPEED, 0);
      break;

      case DIRECTION_UP:
         livAnimate(Planing_Name[current], ANM_MOVE_UP, 0, -1*LS_STD_SCROLL_SPEED);
      break;

      case DIRECTION_DOWN:
         livAnimate(Planing_Name[current], ANM_MOVE_DOWN, 0, 1*LS_STD_SCROLL_SPEED);
      break;
   }
}
	...
	switch (livGetViewDirection(Planing_Name[current]))
	{
		case ANM_MOVE_LEFT:
			tcInsertGuard(list, Planing_GuardRoomList[guard - BurglarsNr],
							  ...
							  14, 14,
							  OL_NR(GetNthNode(PersonsList,guard)), guard + 1 + (PLANING_NR_PERSONS - BurglarsNr), livWhereIs(Planing_Name[current]));
		break;
