			...
			if (choice & INP_ESC)
			{
				...
				*/
				...
				if (!collision)
				{
					...
					{
						if (action = InitAction(plSys, ACTION_GO, (ulong)direction, 0L, 0L))
							PlanChanged = 1;
						else
						    {
...
static void plActionWait(void)
{
	...
	ubyte activ = 0;
	ulong choice1 = 0L, choice2 = 0L, bitset;

	while (activ != PLANING_WAIT_RETURN)
	{
		bitset = BIT(PLANING_WAIT)+
		    BIT(PLANING_WAIT_RETURN);
		...
		case PLANING_WAIT_RADIO:
			if (has(Person_Matt_Stuvysunt, Tool_Funkgeraet))
			{
				...
				if (BurglarsNr > 2)
				{
					...
					node = (NODE *) UnLink(BurglarsList, OL_NAME(GetNthNode(BurglarsList, CurrentPerson)),(void **)&help);
...
static void plLevelDesigner(LSObject lso)
{
	...
	ulong area = lsGetActivAreaID();

	bitset = BIT(PLANING_LD_MOVE) 	+
	    BIT(PLANING_LD_REFRESH) +
	    BIT(PLANING_LD_OK)		+
	    BIT(PLANING_LD_CANCEL);
	...
	while (!ende)
	{
		...
		case PLANING_LD_MOVE:
			while (1)
			{
				long choice = inpWaitFor(INP_RIGHT|INP_LEFT|INP_UP|INP_DOWN|INP_LBUTTONP);
		...
		if (ChoiceOk(choice1, GET_OUT, actionList))
		{
			    ...
			    if ((CurrentPerson >= BurglarsNr) || !CHECK_STATE(lsGetObjectState(choice1), Const_tcIN_PROGRESS_BIT))
			{
				if ((CurrentPerson >= BurglarsNr) || plIgnoreLock(choice1) || CHECK_STATE(lsGetObjectState(choice1), Const_tcLOCK_UNLOCK_BIT))
				{
					...
					{
						if (InitAction(plSys, what, choice1, 0L, opensGet(((LSObject)dbGetObject(choice1))->Type, Tool_Hand) * PLANING_CORRECT_TIME))
						{
							PlanChanged = 1;
							...
							plSync(PLANING_ANIMATE_NO, GetMaxTimer(plSys), opensGet(((LSObject)dbGetObject(choice1))->Type, Tool_Hand) * PLANING_CORRECT_TIME, 1);
...
static void plActionTake(void)
{
	...
	ulong choice = 0L, choice1 = 0L, choice2 = 0L, state;
	char exp[TXT_KEY_LENGTH];
	...
	if (LIST_EMPTY(actionList))
		...
		for (n = (struct ObjectNode *)LIST_HEAD(actionList); NODE_SUCC(n); n = (struct ObjectNode *)NODE_SUCC(n))
		{
			...
			if (!LIST_EMPTY(ObjectList))
			{
				if (CHECK_STATE(state, Const_tcTAKE_BIT))
				{
					...
					{
						for (h2 = (struct ObjectNode *)LIST_HEAD(ObjectList); NODE_SUCC(h2); h2 = (struct ObjectNode *)NODE_SUCC(h2))
						{
							h = (struct ObjectNode *)CreateNode (takeableList, sizeof(struct ObjectNode), OL_NAME(h2));
							h->nr   = OL_NR(h2); /* Loot */
							h->type = OL_NR(n);  /* Original */
		...
		if (!LIST_EMPTY(takeableList))
		{
			...
			{
				ulong weightPerson = tcWeightPersCanCarry((Person)dbGetObject (OL_NR(GetNthNode (BurglarsList, CurrentPerson))));
				ulong volumePerson = tcVolumePersCanCarry((Person)dbGetObject (OL_NR(GetNthNode (BurglarsList, CurrentPerson))));
				...
				if ((Planing_Weight[CurrentPerson]  + weightLoot) <= weightPerson)
				{
					if ((Planing_Volume[CurrentPerson]  + volumeLoot) <= volumePerson)
					{
						if (InitAction(plSys, ACTION_TAKE, choice2, choice1,  PLANING_TIME_TAKE * PLANING_CORRECT_TIME))
						{
							...
							{
								ulong newValue = GetP(dbGetObject(choice2), hasLoot(CurrentPerson), dbGetObject(choice1));

								if (Ask(dbGetObject (OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1)))
								{
									ulong oldValue = GetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1));

									SetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1), oldValue + newValue);
								}
								else
								    SetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1), newValue);
							}
...
static char *plSetUseString(ulong nr, ulong type, void *data)
{
	static char useString[TXT_KEY_LENGTH];
	ulong actLoudness = lsGetLoudness(livGetXPos(Planing_Name[CurrentPerson]), livGetYPos(Planing_Name[CurrentPerson]));
	ulong objLoudness = soundGet(((LSObject)dbGetObject(UseObject))->Type, nr);

	useString[0] = '\0';

	if (break(((LSObject)dbGetObject(UseObject))->Type, nr))
	{
		sprintf(useString, "%ld %s", tcGuyUsesTool(OL_NR(GetNthNode(PersonsList, CurrentPerson)), (Building)dbGetObject(Planing_BldId), nr, ((LSObject)dbGetObject(UseObject))->Type), txtSeconds);
...
static ubyte plCheckAbilities(ulong persId, ulong checkToolId)
{
	NODE *n;
	LIST *requires;
	ubyte ret = 1;
...
static ubyte plCheckRequiredTools (ulong checkToolId)
{
	LIST *trl;
	NODE *n, *h;
	ubyte ret = 1;
...
{
	// fge Stechkarte nur ein, wenn zum aktuellen Zeitpunkt
...
{
	LIST *actionList = plGetObjectsList(CurrentPerson, 0);
	ulong choice1 = 0L, choice2 = 0L, state;
	ubyte i;
	char exp[TXT_KEY_LENGTH];
		...
		else
		{
			...
			else
			{
				...
				if (ChoiceOk(choice1, GET_OUT, actionList))
				{
					...
					{
						ulong newAreaId = StairConnectsGet(choice1, choice1);
					...
					else if (dbIsObject(choice1, Object_Police))
					{
						if (has(OL_NR(GetNthNode(PersonsList, CurrentPerson)), Ability_Kampf))
						{
							...
							if (ChoiceOk(choice2, GET_OUT, objList))
							{
								...
								if (InitAction(plSys, ACTION_USE, choice1, choice2, tcGuyUsesTool(OL_NR(GetNthNode(PersonsList, CurrentPerson)), (Building)dbGetObject(Planing_BldId), choice2, Item_Wache) * PLANING_CORRECT_TIME))
								{
									PlanChanged = 1;
									...
									plWork(CurrentPerson);
									plSync(PLANING_ANIMATE_NO, GetMaxTimer(plSys), tcGuyUsesTool(OL_NR(GetNthNode(PersonsList, CurrentPerson)), (Building)dbGetObject(Planing_BldId), choice2, Item_Wache) * PLANING_CORRECT_TIME, 1);
									livRefreshAll();
								}
								else
									plSay("PLANING_END", CurrentPerson);
						...
						if (!CHECK_STATE(state, Const_tcIN_PROGRESS_BIT))
						{
							...
							else
							{
								if (!CHECK_STATE(state, Const_tcLOCK_UNLOCK_BIT))
								{
									...
									hasAll(Person_Matt_Stuvysunt, OLF_NORMAL|OLF_INCLUDE_NAME|OLF_INSERT_STAR|OLF_ADD_SUCC_STRING|OLF_ALIGNED, Object_Tool);
									...
									if (ChoiceOk(choice2, GET_OUT, ObjectList))
									{
										...
										if (plCheckAbilities(OL_NR(GetNthNode(PersonsList, CurrentPerson)), choice2))
										{
											if (plCheckRequiredTools(choice2))
											{
												...
												{
													if (InitAction(plSys, ACTION_USE, choice1, choice2, tcGuyUsesTool(OL_NR(GetNthNode (PersonsList, CurrentPerson)), (Building)dbGetObject(Planing_BldId), choice2, ((LSObject)dbGetObject(choice1))->Type) * PLANING_CORRECT_TIME))
													{
														PlanChanged = 1;

														plWork(CurrentPerson);
														plSync(PLANING_ANIMATE_NO, GetMaxTimer(plSys), tcGuyUsesTool(OL_NR(GetNthNode(PersonsList, CurrentPerson)), (Building)dbGetObject(Planing_BldId), choice2, ((LSObject)dbGetObject(choice1))->Type) * PLANING_CORRECT_TIME, 1);

														if (!plIgnoreLock(choice1))
														{
															lsSetObjectState(choice1, Const_tcLOCK_UNLOCK_BIT, 1);

															if (((Tool)dbGetObject(choice2))->Effect & Const_tcTOOL_OPENS)
															{
	...
	while (activ != PLANING_ACTION_RETURN)
	{
		if (CurrentPerson < BurglarsNr)
		{
			bitset = BIT(PLANING_PERSON_WALK)+
			    ...
			    BIT(PLANING_ACTION_WAIT)+
			    BIT(PLANING_ACTION_RETURN);
		...
		case PLANING_PERSON_CHANGE:
			...
			if (choice1 != GET_OUT)
			{
				ulong oldTime = CurrentTimer(plSys);
				...
				if (ChoiceOk(choice1, GET_OUT, ObjectList))
				{
					...
					if (choice2 = plGetNextLoot())
					{
						if (InitAction(plSys, ACTION_DROP, choice2, choice1, PLANING_TIME_DROP * PLANING_CORRECT_TIME))
						{
							...
							SetP(dbGetObject(choice2), hasLoot(CurrentPerson), dbGetObject(choice1), GetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1)));
							UnSet(dbGetObject(OL_NR(GetNthNode(BurglarsList, CurrentPerson))), take_RelId, dbGetObject(choice1));
							Planing_Weight[CurrentPerson] -= weightLoot;
							Planing_Volume[CurrentPerson] -= volumeLoot;
...
static void plLook(void)
{
	...
	ulong timer = 0L, maxTimer = GetMaxTimer(plSys), realCurrentPerson = CurrentPerson, choice1;
	ulong bitset;
	...
	while (activ != PLANING_LOOK_RETURN)
	{
		...
		bitset = BIT(PLANING_LOOK_PLAN)+
		    BIT(PLANING_LOOK_RETURN);
	...
	if (livWhereIs(Planing_Name[CurrentPerson]) != lsGetActivAreaID())
	{
		...
		lsInitActivArea(livWhereIs(Planing_Name[CurrentPerson]), livGetXPos(Planing_Name[CurrentPerson]), livGetYPos(Planing_Name[CurrentPerson]), Planing_Name[CurrentPerson]);
	}
...
void plPlaner(ulong objId)
{
	...
	{
		// im Falle Starford Kaserne muá der Plan geladen werden
