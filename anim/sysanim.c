...
struct AnimHandler
{
	ubyte	*RunningAnimID;      /* Anim, die gerade lÑuft */
...
// initialisert diverse Werte und kopiert anschlie·end die Animphasen
// in den XMS Speicher
static void PrepareAnim (char *AnimID)
{
	...
	if ((ulong)(txtCountKey(pict_list)) > PIC_1_ID_POS)
	{
		...
		/* einen Offset mu· ich zur Totalbreite addieren ! Beispiel :	 */
