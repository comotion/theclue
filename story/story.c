...
void tcDoneHotelReception(void)
{
			...
			if (env->MattHasHotelRoom == 0)     /* 1. mal da */
			{
				...
				while ((choice == 2) || (choice == 3))
				{
					...
					case 0:
						...
						if (Say(STORY_0_TXT, 0, MATT_PICTID, "HOTEL_MATT_2") == 1)
						{
							evaluation = 1;
	...
	gfxChangeColors(l_wrp,3,GFX_FADE_OUT,0);
}
...
void tcDoneMeetBriggs(void)
{
	...
	else           /* nicht angenommen ! */
	{
		...
		StopAnim();
		gfxChangeColors(l_wrp, 8, GFX_FADE_OUT, 0);
...
#if 0
void tcDoneAfterMeetingBriggs(void)
{
	SetBubbleType(THINK_BUBBLE);
...
void tcDoneDartJager(void)
{
	...
	if (!Env->MattHasIdentityCard)
	{
		...
		if(choice==0)
		{
			...
			StopAnim();
			gfxChangeColors(l_wrp,3L,GFX_FADE_OUT,0L);
...
void tcDone7thBurglary(void)
{
	...
	/* Jaguar kommt hier, da: 1. man braucht ihn fue\81r Villa, */
...
void tcDoneConfessingSabien(void)
{
	...
	if (Env->WithOrWithoutYou)    /* bleibt bei Sabien! */
	{
		...
		/* hier eventuell glueckliche Anim zeigen! */
...
void tcDoneSouthhampton(void)
{
	...
	SceneArgs.ReturnValue    = 0; /* MUss SEIN! */
	...
	/* mit Gehen oder Planen kommt man aus dem Menue raus */
...
static void tcInitTowerBurglary(void)
{
	...
	car->TyreState = 255;
	car->Strike = 80;       /* extrem niedrig! (fuer diesen Wagen) */
...
static long tcDoTowerBurglary(void)
{
	...
	/* das Organisatorische muss hier wiederholt werden, da */
...
long tcIsLastBurglaryOk(void)
{
	...
	/* Links muss ein, Rechts muss ausgeschalten sein */
	/* 1.. OFF!    */
	...
	/* alle anderen muessen beim Auto sein... */
	for (i = 1; i < 4; i++)
