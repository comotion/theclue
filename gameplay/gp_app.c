...
/* wird von 2 Stellen aufgerufen! (story_9)! */
{
	...
	if (ret)  /* nur wenn Einbruch stattgefunden hat! */
	{
		...
		if (buildingID != Building_Grab_von_Karl_Marx)
		    ...
		    RemTaxiLocation(((Building)dbGetObject(Location_Highgate_Out))->LocationNr);
	...
	case 0:
		...
		succNr = 0;    /* Plan hat nicht funktioniert! */
		break;
	case FAHN_ESCAPED:
		...
		succNr = SCENE_FAHNDUNG;  /*Plan hat funktioniert & */
		break;                   /* Flucht gelungen */
	case FAHN_NOT_ESCAPED:
		sndPlaySound("failed.bk", 0);

		inpDelay(300);

		succNr = SCENE_PRISON;   /* Plan hat funktioniert & */
		break;                   /* Flucht nicht gelungen   */
	default:
		break;
	}
...
void StdInit(void)
{
	...
	if ((sc->LocationNr != -1L) && (GetLocation != sc->LocationNr))
	{
...
void ShowTime(ulong delay)
{
	...
	gfxSetPens(u_wrp, 254, 252, GFX_SAME_PEN);
	gfxSetRect(280, 32);
	gfxSetDrMd(u_wrp, GFX_JAM_1);
...
ulong StdHandle(ulong choice)
{
	...
	switch (choice)
	{
		...
		break;
		case INVESTIGATE:
			Investigate(NODE_NAME (GetNthNode(film->loc_names,(GetCurrentScene()->LocationNr))));
			ShowTime(0);
		break;
		case MAKE_CALL:
			AddVTime(4);
			...
			if (!(GamePlayMode & GP_DEMO))
			{
				...
				else
			    {
					...
					else
				    {
						...
						{
							AddVTime(27153);       /* etwas ueber 15 Tage ! */
							succ_eventnr = tcBurglary(building);
						}
						else
					    {
							AddVTime(443);         /* etwas ueber 7 Stunden */

							CurrentBackground = BGD_LONDON;
							ShowMenuBackground();

							gfxShow (173, GFX_ONE_STEP|GFX_NO_REFRESH|GFX_BLEND_UP, 3, -1, -1);
							tcRefreshLocationInTitle(GetLocation);
...
void tcPersonGreetsMatt(void)
{
	...
	if (CalcRandomNr(0L,upper) == 1)    /* alle upper mal wird Matt gegruext ! */
	{
